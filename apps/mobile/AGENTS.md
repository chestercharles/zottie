...
Route files should control their own navigation options (like `headerShown`, `title`, etc.) by exporting an `options` object. This keeps configuration colocated with the route rather than in `_layout.tsx`.
